...
function HeroSection() {
  return (
    <div
      ...
      className="container w-full relative bg-hero bg-cover bg-center"
    >
      <div className="textstructure py-[10vh]">
        {["The Design", "That", "Inspires"].map((item, index) => (
            ...
            <div className="w-fit flex items-center overflow-hidden">
              {index === 1 && (
                <motion.div
                  ...
                  className="w-[8vw] h-[5vh] sm:h-[6vh] sm:w-[5vw] sm:mt-5 mr-5 rounded-md bg-[#004D43]"
                ></motion.div>
              )}
              <h1
                key={index}
                className="font-FoundersGroteskCondensed sm:-mb-[1vw] md:text-[10vw] sm:text-[12vw] text-[16vw] uppercase whitespace-nowrap leading-none h-full text-[#FFD700]"
              >
                {item}
              </h1>
            </div>
      ...
      <div className="flex items-center justify-between border-t-[1px] border-zinc-800 py-5 font-NueueMontreal">
        {["For The Artists", "By The Artists"].map((item, index) => (
          <p
            key={index}
